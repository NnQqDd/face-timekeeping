...
# Read the database of face embeddings
def read_database():
    ...
    for file_name in os.listdir('database'):
        if file_name.endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
            ...
            faces = FACE_APP.get(avatar)
            if len(faces) == 0:
                print(f'Warning: No face detected in [{file_name}]. Skipping this image.')
                continue
            embedding = faces[0].embedding
            embedding = embedding / np.linalg.norm(embedding)  # Normalize the embedding
            avatars[employee_id].append((embedding, file_name))
    return avatars
...
EMBEDDINGS = []  # Ensure EMBEDDINGS is initialized as an empty list
INDEX_TO_ID = []  # Ensure INDEX_TO_ID is initialized as an empty list

for ID, avatars in AVATARS.items():
    for embedding, file_name in avatars:
